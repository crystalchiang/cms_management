                    ...
                    @endif
                      <form method="POST" action="{{ route('topics.update',$topic->id) }}">
                          ...
                          <div>
                            <div class="form-group">
                                <label>系列</label>
                                <select class="form-control" name="first_cat_id" v-model="first_cat_id" placeholder="請選擇" @change="changeFirstCat(false)" required>
  ...
  var app = new Vue({
    ...
    computed: {
      genAliasName: function(){
        ...
        if(this.third_categories.length > 0){
          this.third_categories.forEach(function(item){
            if(item.id == $this.third_cat_id){
              alias += '-' + item.name;
            }
          });
        }
    ...
    methods: {
      changeFirstCat(init = false){
        ...
        if(init){
          this.second_cat_id = this.topic.second_cat_id;
        }
        this.second_categories.forEach(function(item){
        ...
        if(this.third_categories.length > 0){
          ...
          if(init){
            this.third_cat_id = this.topic.third_cat_id;
          }
        }
      },
      changeSecondCat(){
